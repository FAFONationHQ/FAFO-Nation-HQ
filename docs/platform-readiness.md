...
| --- | --- | --- |
| Authentication | WorkOS Staging configured locally; routes, PKCE, callbacks, sealed sessions, sign-out, identity association, and failure boundaries tested | Owner completes one synthetic Staging signup/email verification; then test recovery and expiry |
| Member profiles | Private edit, preview, public preference, and protected routes implemented; real PostgreSQL repository tests pass | Complete the owner-controlled live Staging lifecycle and add authenticated browser persistence coverage |
...
| Testing | 84 unit, 8 integration (real local PostgreSQL plus installed AuthKit), and 72 Chromium tests pass, including 12 axe scans | Owner-controlled live auth lifecycle, first disposable PostgreSQL CI run, and broader manual accessibility |
...
1. Owner completes one synthetic WorkOS Staging signup from local `/join`, verifies the test email, and returns through `http://localhost:3000/auth/callback`; then verify recovery, replay, expiry, and suspension behavior.
