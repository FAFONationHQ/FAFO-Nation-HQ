...
import {
  ...
  hasEveryPermission,
  permissionsForRoles,
} from "../lib/domain/authorization.ts";
import {
  addMoney,
  createMoney,
...
import {
  defaultMediaPlacement,
  mediaIsHomepageEligible,
} from "../lib/domain/media.ts";
...
check("callsigns normalize deterministically", () => {
  assert.equal(normalizeCallsign("  Maple__Guard  "), "maple-guard");
  assert.deepEqual(validateCallsign("Maple Guard"), {
    valid: true,
    callsign: "maple-guard",
  });
});

check("reserved, malformed, and unsafe callsigns are rejected", () => {
...
check("member eligibility enforces the exact 18+ boundary", () => {
...
check("authorization is default deny and roles aggregate explicit permissions", () => {
...
check("consent is purpose-specific and revocation wins", () => {
...
check("public member projection is allow-listed and location needs separate consent", () => {
  ...
  const withLocation = projectPublicMemberProfile(memberProfile, [
    ...baseConsent,
    {
...
check("public deployments are sanitized and default closed", () => {
...
check("static FAFO World adapter preserves record identity and statistics", () => {
  assert.deepEqual(
    staticFafoWorldRepository.listGearDeployments().map(({ id }) => id),
    [
...
check("money rejects invalid values and prevents cross-currency arithmetic", () => {
...
check("Custom Shop workflow is guest-first and transitions default closed", () => {
...
check("Media homepage placement is manual and rights-gated", () => {
...
check("audit metadata is minimized and secret-like fields are excluded", () => {
...
check("public API failures expose only fixed safe messages", () => {
...
console.log(`Domain invariant checks passed: ${checks}`);
